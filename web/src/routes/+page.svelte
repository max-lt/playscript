	...
	<main class="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-2">
		...
		<section class="flex min-h-0 flex-col">
			{#if result}
				...
				{#if result.truncated}
					<div
						class="border-b border-amber-900/50 bg-amber-950/30 px-4 py-1.5 text-xs text-amber-400"
					>
						Trace truncated at {events.length.toLocaleString('en-US')} events — reduce the input to record
						it all.
					</div>
				{/if}
